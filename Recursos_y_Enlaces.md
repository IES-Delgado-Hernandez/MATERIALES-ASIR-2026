# 📘 Materiales ASIR 2026

Repositorio con apuntes, recursos y plantillas para el alumnado de 2º ASIR, enfocado en el **Proyecto Intermodular** y los módulos relacionados.

---

## 📑 Índice de secciones
- [📚 Normativa oficial](#-normativa-oficial)
- [🗂️ Proyecto Intermodular](#%EF%B8%8F-proyecto-intermodular)
- [📊 Herramientas de gestión de proyectos](#-herramientas-de-gestión-de-proyectos)
- [🌐 Redes y virtualización](#-redes-y-virtualización)
- [🖥️ Sistemas](#%EF%B8%8F-sistemas)
- [🔐 Seguridad y copias de respaldo](#-seguridad-y-copias-de-respaldo)
- [☁️ Cloud y contenedores](#%EF%B8%8F-cloud-y-contenedores)
- [📖 Recursos de aprendizaje adicionales](#-recursos-de-aprendizaje-adicionales)

---

## 📚 Normativa oficial
- 📄 [Ley Orgánica 3/2022, de Ordenación e Integración de la FP](https://www.boe.es/buscar/doc.php?id=BOE-A-2022-4975)  
- 📄 [RD 659/2023 – Ordenación de la FP](https://www.boe.es/buscar/doc.php?id=BOE-A-2023-12955)  
- 📄 [Orden EFD/659/2024 – Desarrollo curricular FP](https://www.boe.es/diario_boe/txt.php?id=BOE-A-2024-9251)  
- 📄 [Título oficial ASIR (RD 1629/2009)](https://www.boe.es/buscar/doc.php?id=BOE-A-2009-18356)  

---

## 🗂️ Proyecto Intermodular
- 📘 [Guía básica de Git y GitHub (cheat sheet)](https://education.github.com/git-cheat-sheet-education.pdf)  
- 📘 [Ejemplo de memoria de proyecto de FP (formato oficial, Junta de Andalucía)](https://www.juntadeandalucia.es/educacion/portals/web/educacion-profesional/proyecto-intermodular) *(siempre adaptado a centro)*  
- 📘 [Buenas prácticas para documentar proyectos](https://guides.github.com/features/wikis/)  

---

## 📊 Herramientas de gestión de proyectos
- 🗓️ [Crear diagramas de Gantt online – GanttProject (gratis)](https://www.ganttproject.biz/)  
- 🗓️ [Apuntes Diagrama de Gantt](https://face.unt.edu.ar/web/iadmin/wp-content/uploads/sites/2/2014/12/Aplicaci%C3%B3n-pr%C3%A1ctica-Diagrama-de-Gantt-para-Jornada-IA-Handl.pdf)  
- 📐 [Draw.io – Diagramas de red y UML](https://app.diagrams.net/)  
- 📐 [Lucidchart (alternativa online)](https://www.lucidchart.com/pages/es)  

---

## 🌐 Redes y virtualización
- 🌐 [Documentación oficial de Cisco Packet Tracer](https://www.netacad.com/courses/packet-tracer)  
- 🌐 [Manual de pfSense](https://docs.netgate.com/pfsense/en/latest/)  
- 🌐 [Proxmox VE – Virtualización](https://pve.proxmox.com/wiki/Main_Page)  
- 🌐 [VMware ESXi documentación](https://docs.vmware.com/es/VMware-vSphere/index.html)  

---

## 🖥️ Sistemas
- 💻 [Active Directory – Microsoft Docs](https://learn.microsoft.com/es-es/windows-server/identity/ad-ds/get-started/virtual-dc/active-directory-domain-services-overview)  
- 💻 [Linux System Administration Basics](https://tldp.org/LDP/intro-linux/html/)  
- 💻 [Guía de instalación de servicios en Ubuntu Server](https://ubuntu.com/server/docs)  

---

## 🔐 Seguridad y copias de respaldo
- 🔐 [OWASP Top 10 – Buenas prácticas de seguridad](https://owasp.org/www-project-top-ten/)  
- 🔐 [Guía de Seguridad CCN-CERT (España)](https://www.ccn-cert.cni.es/publico/)  
- 💾 [Estrategias de backup 3-2-1 (Veeam)](https://www.veeam.com/blog/es/estrategia-de-backup-3-2-1.html)  

---

## ☁️ Cloud y contenedores
- ☁️ [Documentación de AWS](https://docs.aws.amazon.com/)  
- ☁️ [Documentación de Microsoft Azure](https://learn.microsoft.com/azure/)  
- ☁️ [Docker Docs](https://docs.docker.com/)  
- ☁️ [Kubernetes Documentation](https://kubernetes.io/docs/home/)  

---

## 📖 Recursos de aprendizaje adicionales
- 📖 [Man pages de Linux online](https://man7.org/linux/man-pages/)  
- 📖 [Cheat sheet de comandos Linux](https://files.fosswire.com/2007/08/fwunixref.pdf)  
- 📖 [Markdown Guide (para documentar bien en GitHub)](https://www.markdownguide.org/basic-syntax/)  
